#ifdef EULER3D_C++_CPP_AERODYNAMICPARAMETERS_CPP
#define EULER3D_C++_CPP_AERODYNAMICPARAMETERS_CPP
...
// Calculate pressure coefficient for each cell
  ...
  if(iter_ == iteration_interval_ || solver->stop_solver_flag_ == true)
  {
    ...
    for(i=0; i < block_wall_face_ids_; i++)
    {
      // Find wall cells associate with the wall face i
      cell_0 = block->block_faces[]->face_2_cells_connectivity_[0];
      cell_1 = block->block_faces[]->face_2_cells_connectivity_[1];

      // Pressure value from cell 0 and 1
      pp0 = block->primitivevariables->pp_[cell_0];
      pp1 = block->primitivevariables->pp_[cell_1];
...
void AerodynamicParameters::calculateMach(Block* block, PostProcessing* postprocessing, double gamma_)
{
  ...
  {
    // Primitve variables
    pp_ = block->primitive_variables_->pp_[i];
    ro_ = block->primitive_variables_->ro_[i];
    uu_ = block->primitive_variables_->uu_[i];
    vv_ = block->primitive_variables_->vv_[i];
    ww_ = block->primitive_variables_->ww_[i];
...
void AerodynamicParameters::calculateCp(Block* block, PostProcessing* postprocessing)
{
  ...
  for(i=0; i < block->n_cells_in_block_ ; i++)
  {
    ...
    pp_ = block->primitivevariables->pp_[i];
